 ...
 * Node, and will show different FCK editors for the various languages
 * configured.
 *
 ...
 *
 */
class MlFckAttribute extends MlTextAttribute
{
    var $m_editorname = "";
    var $m_urlpath = "";
    var $fck_opt = array(
    ...
    {
        $config_fck = Config::getGlobal('fck');
        ...
        $this->fck_opt["Language"] = Config::getGlobal("language");
    ...
    {
        include_once(Config::getGlobal('atkroot') . "atk/attributes/fck/fckeditor.php");
    ...
    public function addToEditArray($mode, &$arr, &$defaults, &$error, $fieldprefix)
    {
        ...
        for ($i = 1; $i < $langCount; $i++) {
            ...
            $entry["label"] = $this->label($defaults) .
                ...
                strtolower($curlng) . '">' .
                Tools::atktext("language_" . strtolower($curlng)) .
                '</label>)';
    ...
    public function editFck($record = "", $fieldprefix = "", $fieldname = "")
    {
        ...
        $oFCKeditor->BasePath = Config::getGlobal("atkroot") . "atk/attributes/fck/";
