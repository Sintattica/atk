...
Tools::useattrib("atkmlattribute");
 ...
 *
 */
class MlSelectorAttribute extends MlAttribute
{
    ...
    function getSelector($prefix = "")
    {
        ...
        if (!is_array($languages) || sizeof($languages) == 0) {
            return Tools::atktext("multilanguage_error_config", $this->m_ownerInstance->m_module,
        ...
        $result .= '<select id="' . $prefix . '_lgswitch" name="' . $prefix . '_lgswitch" onchange="changeLanguage(this, \'' . $prefix . '\', ' . (Config::getGlobal("multilanguage_linked")
        ...
        for ($i = 1; $i < sizeof($languages); $i++) {
            $result .= '<option value="' . $languages[$i] . '" ' . (strtolower($languages[$i]) == strtolower($this->m_ownerInstance->m_postvars['atkeditlng'])
                    ? 'selected' : '') . '>' . Tools::atktext('language_' . strtolower($languages[$i])) . '</option>';
        }
        ...
        if (!$s_wroteStrings) {
            ...
            for ($i = 0, $_i = count($languages); $i < $_i; $i++) {
                $script .= "str_languages['" . $languages[$i] . "'] = '" . Tools::atktext('language_' . strtolower($languages[$i])) . "';\n";
            }
    ...
    function edit($record = "", $prefix = "", $mode = "")
    {
        /* register javascript */
        $page = Page::getInstance();
        $page->register_script(Config::getGlobal("atkroot") . "atk/javascript/class.atkmultilanguage.js.php");
