...
/**
 * The atkMlAttribute class represents an multilanguage attribute of an Node.
 *
 ...
 *
 */
class MlAttribute extends Attribute
{
    var $m_language = "";
    ...
    function atkMlAttribute($name, $flags = 0, $size = 0)
    {
        ...
        $this->m_language = strtoupper(Config::getGlobal("defaultlanguage"));
    }
    ...
    {
        return Config::getGlobal("supported_languages");
    }
    ...
    function addToEditArray($mode, &$arr, &$defaults, &$error, $fieldprefix)
    {
        ...
        if ($this->m_edited) { // if we edited this attrib, we also need to add the
            ...
            /* label? */
            $entry["label"] = $this->label($defaults) . ' (<label id="' . $fieldprefix . $this->formName() . '_label">' . Tools::atktext("language_" . strtolower($curlng)) . '</label>)';
