...
ATK.DateAttribute = {
    ...
    notifyListeners: function (name) {
        ...
        for (var i = 0; i < listeners.length; i++) {
            listeners[i]({
                name: name, value: ATK.DateAttribute.getValue(name)
            });
        }
    },
    /**
    ...
    setValue: function (name, value, fromCalendar) {
        ...
        ATK.DateAttribute.adjustDate(yearEl, name, value);
        ATK.DateAttribute.adjustDate(monthEl, name, value);
        ATK.DateAttribute.adjustDate(dayEl, name, value);
     ...
     */
    adjustDate: function (el, arr, newDate) {
        ...
        var date_minimum = new Date(minimum["y"], minimum["m"] - 1, minimum["d"]);
        var date_maximum = new Date(maximum["y"], maximum["m"] - 1, maximum["d"]);
        ...
        if (current["d"] != 0 && current["y"] != 0 && current["m"] != 0) {
            ...
            date_current.getMonth() == date_maximum.getMonth()
                ? date_maximum.getDate() : ATK.DateAttribute.getDays(date_current));
