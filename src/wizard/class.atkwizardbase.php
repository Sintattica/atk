        ...
        if ($this->m_mode == WIZARD_MODE_ADD) {
            ...
            if ($node->m_action != 'admin') {
                ...
                if ($this->showFinishButton()) {
                    $result[] = '<input type="submit" class="btn_next" name="atkwizardaction[finish]" value="' . Atk_Tools::atktext("finish",
                            "atk") . '">';
                } else {
                    $result[] = '<input type="submit" class="btn_next" name="atkwizardaction[' . $atkwizardaction . ']" value="' . Atk_Tools::atktext("next",
                            "atk") . '">';
                }
            } else {
                //if we explicitly don't want the finish button we set a hidden var to post the atkwizardaction
                ...
                $result[] = '<input type="submit" class="btn_next" name="atkwizardaction[saveandaddnew]" value="' . Atk_Tools::atktext("saveandaddnew",
                        "atk") . '">';
                if ($this->showFinishButton()) {
                    $result[] = '<input type="submit" class="btn_next" name="atkwizardaction[finish]" value="' . Atk_Tools::atktext("finish",
                            "atk") . '">';
                } else {
                    $result[] = '<input type="submit" class="btn_next" name="atkwizardaction[' . $atkwizardaction . ']" value="' . Atk_Tools::atktext("saveandnext",
                            "atk") . '">';
                }
            }

            $result[] = '<input type="submit" class="btn_cancel" name="atkwizardcancel" value="' . Atk_Tools::atktext("cancel",
                    "atk") . '">';
        } elseif ($this->m_mode == WIZARD_MODE_EDIT) {
            // We post the action as key value in de atkwizardaction var. Therefor
            // we have to convert the atkwizardaction value in Atk_Wizard::start().        
            $result[] = '<input type="submit" class="btn_save" name="atknoclose" value="' . Atk_Tools::atktext("save",
                    "atk") . '">';
            $result[] = '<input type="submit" class="btn_next" name="atkwizardaction[finish]" value="' . Atk_Tools::atktext("finish",
                    "atk") . '">';
            $result[] = '<input type="submit" class="btn_cancel" name="atkcancel" value="' . Atk_Tools::atktext("cancel",
                    "atk") . '">';
        } else {
