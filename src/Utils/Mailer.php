...
class Mailer extends PHPMailer
{
    ...
    {
        $charset = strtoupper(Tools::atkGetCharset());
    ...
    function Send()
    {
        if (Config::getGlobal("mail_enabled", true)) {
            // make sure Sender is set so the Return-Path header will have a decent value
            if ($this->Sender == "") {
                $this->Sender = $this->From;
            }

            $mail_redirect = Config::getGlobal("mail_redirect");
    ...
    function recipientFieldToString($field)
    {
        ...
        foreach ($field as $i => $recipient) {
            ...
            if (Tools::atk_strlen($recipient['1']) > 0) {
