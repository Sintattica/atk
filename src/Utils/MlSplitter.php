...
class MlSplitter
{
     ...
     * @return MlSplitter The instance.
     */
    public static function &getInstance()
    {
        static $s_mlsplitter = null;
        if (!is_object($s_mlsplitter)) {
            $s_mlsplitter = new self();
            Tools::atkdebug("Created a new atkmlsplitter instance");
        }
        return $s_mlsplitter;
    }
    ...
    function getLanguages(&$node)
    {
        ...
        $lngs = Config::getGlobal("supported_languages");
    ...
    {
        Tools::atkdebug("mlsplitter::updateMlRecords() for mode $mode");
        ...
        foreach ($node->m_attribList as $attribname => $attrib) {
            if (is_subclass_of($node->m_attribList[$attribname], "atkRelation")) {
                ...
                if ((is_a($node->m_attribList[$attribname],
                            ...
                            "atkOneToOneRelation")) && Tools::hasFlag($node->m_attribList[$attribname]->storageType(),
                        ADDTOQUERY)
                ) {
                    ...
                    $p_attrib = new Attribute($attribname);
        ...
        $atklngrecordmodes = SessionManager::sessionLoad("atklng_" . $node->m_type . "_" . SessionManager::atkPrevLevel());
        ...
        SessionManager::sessionStore("atklng_" . $node->m_type . "_" . SessionManager::atkPrevLevel(),
            null); // deleting modes
    }
    ...
    function combineMlRecordSet(&$node, &$recordset, $query)
    {
        ...
        SessionManager::sessionStore("atklng_" . $node->m_type . "_" . SessionManager::atkLevel(),
    ...
    function addLngRecords(&$node, &$recordset)
    {
        $newrecordset = Array();
        $languages = $this->getLanguages($node);
        Tools::atkdebug("atkmlsplitter adding missings lngrecord for " . $node->m_type . "!");
