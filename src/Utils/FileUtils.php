...
class FileUtils
{
    ...
    function copyDirRecursive($source, $dest, $dirname = '', $privileges = 0777)
    {
        ...
        Tools::atkdebug("Checking write permission for " . $orgdest);

        if (!FileUtils::is_writable($orgdest)) {
            Tools::atkdebug("Error no write permission!");
            return false;
        }

        Tools::atkdebug("Permission granted to write.");

        if ($dest == $orgdest && $dirname != '') {
            mkdir($orgdest . "/" . $dirname, $privileges);
            return FileUtils::copyDirRecursive($source, $orgdest . "/" . $dirname, '', $privileges);
        }
        ...
        while (false !== $entry = $dir->read()) {
            ...
            if ($dest !== "$source/$entry") {
                FileUtils::copyDirRecursive("$source/$entry", "$dest/$entry", $dirname, $privileges);
            }
        }
    ...
    function is_writable($orgdest)
    {
        ...
        Tools::atkdebug("Checking with: " . $testdest);
    ...
    function mkdirRecursive($dir, $privileges = 0777, $recursive = true)
    {
        ...
        if (FileUtils::mkdirRecursive(dirname($dir), $privileges, $recursive)) {
    ...
    {
        $stringparser = new StringParser($template);
    ...
    function atkGetMimeTypeFromFileExtension($filename)
    {
        ...
        if (array_key_exists($ext, $mimetypes)) {
            Tools::atkdebug("Filetype for $filename is {$mimetypes[$ext]}");
            return $mimetypes[$ext];
        } else {
            Tools::atkdebug("Filetype for $filename could not be found. Returning application/octet-stream.");
    ...
    function rmdirRecursive($dir)
    {
        ...
        while (false !== ($entry = $d->read())) {
            ...
            $entry = $dir . '/' . $entry;
            if (is_dir($entry)) {
                if (!FileUtils::rmdirRecursive($entry)) {
                    return false;
                }
                continue;
            }
