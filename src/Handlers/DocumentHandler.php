...
class DocumentHandler extends ActionHandler
{
    ...
    function action_document()
    {
        ...
        if ($this->m_postvars["atkselector"] == "") {
            Tools::atkerror("Selector parameter not available.");
            return false;
        }

        // ATKDocTpl must be available to perform this action
        if (!isset($this->m_postvars["atkdoctpl"])) {
            Tools::atkerror("atkdoctpl parameter not available.");
            return false;
        }
        ...
        if (ereg("[<>\\/|;]", $module . $node . $this->m_postvars["atkdoctpl"]) !== false) {
            Tools::atkerror("Invalid filename given.");
            return false;
        }

        // Check if the file exists
        if (!is_file($tpl_file)) {
            Tools::atkerror("Given file does not exist.");
            return false;
        }
