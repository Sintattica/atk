...
class MySqlDb extends Db
{
    ...
    function __construct()
    {
        if (!function_exists('mysql_connect')) {
            Tools::atkerror('MySQL not supported by your PHP version');
    ...
    function locktables_fallback_on_error($query, $querymode = 'w')
    {
        ...
        if (is_array($matches) && sizeof($matches) == 2) {
            Tools::atkdebug("<b>Fallback feature called because error '1100' occured during the last query. Running query again using table lock for table '{$matches[1]}' (" . $this->m_link_id . ").</b>");
            $table = $matches[1];
    ...
    {
        /* goto next record */
        $this->m_record = @mysql_fetch_array($this->m_query_id, MYSQL_ASSOC | Config::getGlobal("mysqlfetchmode"));
        $this->m_row++;
        $this->m_errno = mysql_errno();
        $this->m_error = mysql_error();
        ...
        if ($columnType === 'decimal') {
            ...
            if (isset($metaData[$tableName][$columnName]['Type'])) {
                ...
                if (($open = strpos($type, '(')) !== false) {
                    ...
                    Tools::atkwarning('Unexpected column type format, falling back on mysql_field_len()');
                }
            }
        }
