...
class clsTbsDataSource
{
    ...
    {
        return $this->oTBS->meth_Misc_Alert('MergeBlock ' . $this->oTBS->ChrOpen . $this->BlockName . $this->oTBS->ChrClose,
            $Msg);
    }
        ...
        if ($this->Type === 7) {
            ...
            if ($Ok) {
                ...
                if (function_exists($this->FctFetch)) {
                    if (!function_exists($this->FctClose)) {
                        $this->Type = $this->DataAlert('The expected custom function \'' . $this->FctClose . '\' is not found.');
                    }
                } else {
                        ...
                        if (is_array($Var)) {
                            ...
                            while ($Ok and ($KeyNum <= $KeyMax)) {
                                if (isset($Var[$Keys[$KeyNum]])) {
                                    ...
                                    $KeyNum++;
                                    if (!is_array($Var)) {
                                        $Ok = $this->DataAlert('Invalid query \'' . $Query . '\' because item \'' . $VarName . '[' . implode('][',
                                                array_splice($Keys, 0, $KeyNum)) . ']\' is not an array.');
                                    }
                                } else {
    ...
    function DataFetch()
    {
        ...
        if ($this->RecSaved) {
            if ($this->RecNum < $this->RecNbr) {
                ...
                if (!is_array($this->CurrRec)) {
                    $this->CurrRec = array('key' => $this->RecKey, 'val' => $this->CurrRec);
                }
                $this->RecNum++;
                if ($this->OnDataOk) {
                    call_user_func_array($LocR->OnDataInfo, $LocR->OnDataPrm);
                }
            } else {
        ...
        switch ($this->Type) {
            ...
            case 7: // Custom function
                ...
                break;
            case 8: // PostgreSQL
                $this->CurrRec = @pg_fetch_array($this->RecSet, $this->RecNum,
                    PGSQL_ASSOC); // warning comes when no record left.
                break;
            case 9: // SQLite
...
// Public methods
    function LoadTemplate($File, $HtmlCharSet = '')
    {
        // Load the file
        $x = '';
        if (!tbs_Misc_GetFile($x, $File)) {
            return $this->meth_Misc_Alert('LoadTemplate Method', 'Unable to read the file \'' . $File . '\'.');
        }
        // CharSet analysis
        if ($HtmlCharSet === '+') {
            $this->Source .= $x;
    ...
    function Show($Render = '')
    {
        ...
        if ($this->_CacheFile !== true) {
            $this->meth_Merge_Special('onshow,var');
        }
        if (is_string($this->_CacheFile)) {
            $this->meth_Cache_Save($this->_CacheFile, $this->Source);
        }
        ...
        do {
            ...
            if ($Pos === false) {
                ...
                if ($ReadPrm) {
                    ...
                    if ($PosEnd === false) {
                        $this->meth_Misc_Alert('Tag definition',
...
// This position can be useful because we don't know in advance how $Value will be replaced.
                ...
                if (is_array($Value)) {
                    if (isset($Value[$x])) {
                        ...
                        if (!isset($Loc->PrmLst['noerr'])) {
                            $this->meth_Misc_Alert('Array value',
                                'Can\'t merge ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' because sub-item \'' . $x . '\' is not an existing key in the array.',
                                true);
                        }
                    }
                    $SubId++;
                    ...
                    if (method_exists($Value, $x)) {
                        ...
                        if (!isset($Loc->PrmLst['noerr'])) {
                            $this->meth_Misc_Alert('Object value',
                                'Can\'t merge ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' because \'' . $x . '\' is neither a method nor a property in the class \'' . get_class($Value) . '\'.',
                                true);
                        }
                    ...
                    if (isset($Loc->PrmLst['selected'])) {
                        ...
                        if (!isset($Loc->PrmLst['noerr'])) {
                            $this->meth_Misc_Alert('Object or Array value expected',
                                'Can\'t merge ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' because the item before \'' . $x . '\' is neither an object nor an array. Its type is ' . gettype($Value) . '.',
                                true);
                        }
                    }
                    $Value =& tbs_Misc_UnlinkVar('');
        ...
        if (isset($Loc->PrmLst['file'])) {
            ...
            if (tbs_Misc_GetFile($CurrVal, $File)) {
                ...
                $CurrVal = '';
                if (!isset($Loc->PrmLst['noerr'])) {
                    $this->meth_Misc_Alert('Parameter \'file\'',
                        'Field ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' : unable to read the file \'' . $File . '\'.',
                        true);
                }
            }
            $Loc->ConvHtml = false;
        ...
        if (isset($Loc->PrmLst['onformat'])) {
            if ($Loc->FirstMerge) {
                ...
                if (!$this->meth_Misc_UserFctCheck($Loc->OnFrmInfo, $ErrMsg)) {
                    ...
                    if (!isset($Loc->PrmLst['noerr'])) {
                        $this->meth_Misc_Alert('Parameter \'onformat\'', $ErrMsg);
                    }
        ...
        if (isset($Loc->PrmLst['if'])) {
            if ($Loc->FirstMerge) {
                ...
                if (isset($Loc->PrmLst['then'])) {
                    $this->meth_Merge_PhpVar($Loc->PrmLst['then'], true);
                }
                if (isset($Loc->PrmLst['else'])) {
                    $this->meth_Merge_PhpVar($Loc->PrmLst['else'], true);
                }
            }
            $x = str_replace($this->ChrVal, $CurrVal, $Loc->PrmLst['if']);
            if (tbs_Misc_CheckCondition($x)) {
                if (isset($Loc->PrmLst['then'])) {
                    $CurrVal =& tbs_Misc_UnlinkVar('' . $Loc->PrmLst['then']);
                } // Now $CurrVal and $OVal are different
            } else {
                $Script = false;
                if (isset($Loc->PrmLst['else'])) {
        ...
        if ($Script) {// Include external PHP script
            ...
            if ($x === false) {
                if (!isset($Loc->PrmLst['noerr'])) {
                    $this->meth_Misc_Alert('Parameter \'script\'',
                        'Field ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' cannot be merged because file \'' . $File . '\' is not found or not readable.',
                        true);
                }
            }
        ...
        if ($Loc->ConvSpe) { // Using special parameters
            if ($Loc->ConvLook) {
                ...
                if ($Loc->ConvHtml === false) {
                    $OVal = tbs_Html_GetPart($OVal, 'BODY', false, true);
                }
            }
            if ($Loc->ConvHtml) {
            ...
            if ($Loc->ConvWS) {
                ...
                do {
                    ...
                    if ($pos !== false) {
                        $OVal = substr_replace($OVal, $nbsp, $pos, 1);
                    }
                } while ($pos !== false);
            }
            if ($Loc->ConvJS) {
        ...
        if ($Loc->ConvProtect) {
            $OVal = str_replace($this->ChrOpen, $this->ChrProtect, $OVal);
        }
        if ($CurrVal !== $OVal) {
            $CurrVal = str_replace($this->ChrVal, $OVal, $CurrVal);
        }
        ...
        if ($Select) {
            tbs_Html_MergeItems($Txt, $Loc, $CurrVal, $SelArray, $NewEnd);
        }
...
// If $Special==true => don't set ->BlockSrc and accept TBS Fields (used for automatic blocks)
        ...
        if ($Block === 'begin') { // Block definied using begin/end
            ...
            return $this->meth_Misc_Alert('Block definition',
                $this->ChrOpen . $Loc->FullName . $this->ChrClose . '] has a \'begin\' tag, but no \'end\' tag found.');
        ...
        if ($Special) {
            ...
            if (tbs_Locator_EnlargeToTag($Txt, $Loc, $Block, true, false) === false) {
                return $this->meth_Misc_Alert('Block definition',
                    $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' can not be defined because tag <' . $Loc->PrmLst['block'] . '> or </' . $Loc->PrmLst['block'] . '> is not found.');
            }
            ...
            if ($Loc->SubOk) {
                ...
                $Loc->BlockSrc = substr($Txt, $Loc->PosBeg, $PosBeg1 - $Loc->PosBeg) . substr($Txt, $PosEnd1 + 1,
                        $Loc->PosEnd - $PosEnd1);
            }
        }
            ...
            if ($QueryOk) {
                if ($Src->Type === 4) { // Special for Text merge
                    if ($LocR->BlockFound) {
                        ...
                        $Src->CurrRec = false;
                        if ($NoFct === false) {
                            $LocR->OnSecPrm[1] =& $Src->CurrRec;
                            $LocR->OnSecPrm[2] =& $Src->RecSet;
                            $LocR->OnSecPrm[3] =& $Src->RecNum;
                    ...
                    if ($LocR->BlockFound !== false) {
                        $this->meth_Merge_BlockSections($Txt, $LocR, $Src, $NoFct, $RecSpe, $RecStop);
                    }
    ...
    function meth_Merge_BlockSections(&$Txt, &$LocR, &$Src, &$NoFct, &$RecSpe, &$RecStop)
    {
        ...
        while ($Src->CurrRec !== false) {
            ...
            if ($GrpFound) {
                ...
                if ($LocR->FooterFound) {
                    ...
                    for ($i = $LocR->FooterNbr; $i >= 1; $i--) {
                        ...
                        if ($Src->RecNum === 1) {
                            ...
                            if (!$change_i) {
                                $change_i = !($LocR->FooterPrevValue[$i] === $x);
                            }
                            if ($change_i) {
                                $grp_change = true;
                                $grp_src = $this->meth_Merge_SectionNormal($LocR, $LocR->FooterBid[$i], $PrevRec,
                                        $PrevNum, $PrevKey, $NoFct) . $grp_src;
                                $LocR->FooterPrevValue[$i] = $x;
                            }
                        }
                    }
                ...
                if ($LocR->HeaderFound) {
                    ...
                    for ($i = 1; $i <= $LocR->HeaderNbr; $i++) {
                        ...
                        if (!$change) {
                            $change = !($LocR->HeaderPrevValue[$i] === $x);
                        }
                        if ($change) {
                            $grp_src .= $this->meth_Merge_SectionNormal($LocR, $LocR->HeaderBid[$i], $Src->CurrRec,
                                $Src->RecNum, $Src->RecKey, $NoFct);
            ...
            if ($SerialMode) { // Serial Merge
                ...
                $SerialTxt[$SerialNum] = $this->meth_Merge_SectionNormal($LocR, $Bid, $Src->CurrRec, $Src->RecNum,
                    $Src->RecKey, $NoFct);
                ...
                if ($LocR->WhenFound) { // With conditional blocks
                    $x = $this->meth_Merge_SectionNormal($LocR, $Bid, $Src->CurrRec, $Src->RecNum, $Src->RecKey,
                        $NoFct);
                    $found = false;
                    $continue = true;
                    $i = 1;
                    do {
                        $cond = $this->meth_Merge_SectionNormal($LocR, $LocR->WhenCondBid[$i], $Src->CurrRec,
                            $Src->RecNum, $Src->RecKey, $NoFct);
                        if (tbs_Misc_CheckCondition($cond)) {
                            $x_when = $this->meth_Merge_SectionNormal($LocR, $LocR->WhenSectionBid[$i], $Src->CurrRec,
                                $Src->RecNum, $Src->RecKey, $NoFct);
                            if ($LocR->WhenBeforeNS[$i]) {
                                $x = $x_when . $x;
                            } else {
                                $x = $x . $x_when;
                            }
                    ...
                    if (($found === false) and ($LocR->WhenDefaultBid !== false)) {
                        $x_when = $this->meth_Merge_SectionNormal($LocR, $LocR->WhenDefaultBid, $Src->CurrRec,
                            $Src->RecNum, $Src->RecKey, $NoFct);
                    ...
                    $BlockRes .= $this->meth_Merge_SectionNormal($LocR, $Bid, $Src->CurrRec, $Src->RecNum, $Src->RecKey,
                        $NoFct);
                }
            }
        ...
        if ($LocR->FooterFound) {
            if ($Src->RecNum > 0) {
                for ($i = 1; $i <= $LocR->FooterNbr; $i++) {
                    if ($LocR->FooterIsFooter[$i]) {
                        $BlockRes .= $this->meth_Merge_SectionNormal($LocR, $LocR->FooterBid[$i], $PrevRec, $PrevNum,
                            $PrevKey, $NoFct);
                    }
                }
            }
        ...
        // NoData
        if (($Src->RecNum === 0) and ($LocR->NoDataBid !== false)) {
            $BlockRes = $LocR->BlockSrc[$LocR->NoDataBid];
        }
            ...
            if ($Loc->SubNbr > 0) {
                if ($Loc->SubLst[0] === '') {
                    ...
                    if ($ObjOk) {
                        ...
                        $this->meth_Misc_Alert('Merge ObjectRef sub item',
                            'Can\'t merge ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' because property ObjectRef is neither an object nor an array. Its type is \'' . gettype($this->ObjectRef) . '\'.',
                            true);
                        $Pos = $Loc->PosEnd + 1;
                    }
                    ...
                    if (isset($Loc->PrmLst['noerr'])) {
                        ...
                        $this->meth_Misc_Alert('Merge PHP global variables',
                            'Can\'t merge ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' because allowed prefix is set to \'' . $Pref . '\'.',
                            true);
                        $Pos = $Loc->PosEnd + 1;
                    }
                    ...
                    if (isset($Loc->PrmLst['noerr'])) {
                        ...
                        $Pos = $Loc->PosEnd + 1;
                        $this->meth_Misc_Alert('Merge PHP global variables',
                            'Can\'t merge ' . $this->ChrOpen . $Loc->FullName . $this->ChrClose . ' because there is no PHP global variable named \'' . $Loc->SubLst[0] . '\'.',
                            true);
                    }
                }
            }
                ...
                case 'tplvars':
                    if ($Loc->SubNbr == 2) {
                        ...
                        if (isset($this->TplVars[$Loc->SubLst[2]])) {
                            ...
                            $this->meth_Misc_Alert('System Fields',
                                'Can\'t merge [' . $Loc->FullName . '] because property TplVars doesn\'t have any item named \'' . $Loc->SubLst[2] . '\'.');
                            return $Loc->PosBeg + 1;
                        }
                    }
                case '':
                    $this->meth_Misc_Alert('System Fields',
                        'Can\'t merge [' . $Loc->FullName . '] because it doesn\'t have any requested keyword.');
                    return $Loc->PosBeg + 1;
                default:
                    $this->meth_Misc_Alert('System Fields',
                        'Can\'t merge [' . $Loc->FullName . '] because \'' . $Loc->SubLst[1] . '\' is an unknown keyword.');
                    return $Loc->PosBeg + 1;
            }
        } else {
            $this->meth_Misc_Alert('System Fields',
                'Can\'t merge [' . $Loc->FullName . '] because it doesn\'t have any subname.');
            return $Loc->PosBeg + 1;
        }
...
// Proceed to one of the special merge
        ...
        foreach ($TypeLst as $Type) {
            switch ($Type) {
                case 'var':
                    $this->meth_Merge_PhpVar($this->Source, true);
                    break;
                case 'onload':
                    $this->meth_Merge_Auto($this->Source, 'onload', true, true);
                    break;
                case 'onshow':
                    $this->meth_Merge_Auto($this->Source, 'onshow', false, true);
                    break;
            }
        }
    ...
    function meth_Merge_SectionNormal(&$LocR, &$BlockId, &$CurrRec, &$RecNum, &$RecKey, &$NoFct)
    {
        ...
        if ($NoFct) {
            ...
            $Txt0 = $Txt;
            $LocR->OnSecPrm[1] =& $CurrRec;
            $LocR->OnSecPrm[2] =& $Txt;
            $LocR->OnSecPrm[3] =& $RecNum;
            call_user_func_array($LocR->OnSecInfo, $LocR->OnSecPrm);
            if ($Txt0 === $Txt) {
                $LocLst =& $LocR->BlockLoc[$BlockId];
                $iMax = $LocLst[0];
        ...
        if ($RecNum === false) { // Erase all fields
            ...
            if ($DoUnCached) {
                ...
                $Name = $BlockName . '.#';
                while ($Loc = $this->meth_Locator_FindTbs($Txt, $Name, $Pos, '.')) {
                    $Pos = $this->meth_Locator_Replace($Txt, $Loc, $RecNum, true);
                }
                $Pos = 0;
                $Name = $BlockName . '.$';
                while ($Loc = $this->meth_Locator_FindTbs($Txt, $Name, $Pos, '.')) {
                    $Pos = $this->meth_Locator_Replace($Txt, $Loc, $RecKey, true);
                }
            }
    ...
    function meth_Merge_SectionSerial(&$LocR, &$SecId, &$SerialNum, &$SerialMax, &$SerialTxt)
    {
        ...
        for ($i = $SerialMax; $i > 0; $i--) {
            ...
            if ($Sr > $SerialNum) {
                ...
                $Txt = substr_replace($Txt, $EmptySrc, $LocLst[$Sr]->PosBeg,
                    $LocLst[$Sr]->PosEnd - $LocLst[$Sr]->PosBeg + 1);
            } else {
                $Txt = substr_replace($Txt, $SerialTxt[$Sr], $LocLst[$Sr]->PosBeg,
                    $LocLst[$Sr]->PosEnd - $LocLst[$Sr]->PosBeg + 1);
            }
        }
...
// onload - onshow
    ...
    function meth_Merge_NavigationBar(&$Txt, $BlockName, $Options, $PageCurr, $RecCnt, $PageSize)
    {
        ...
        // Prepare options
        if (!is_array($Options)) {
            $Options = array('navsize' => intval($Options));
        }
            ...
            if ($Switch) {
                ...
                if (is_string($File) and (strlen($File) > 0)) {
                    ...
                    if (tbs_Misc_GetFile($this->Source, $File)) {
                        ...
                        if (!isset($Loc->PrmLst['noerr'])) {
                            $this->meth_Misc_Alert('Parameter subtpl', 'Unable to read the file \'' . $File . '\'.');
                        }
                    }
                }
            }
...
// This function return the formated representation of a Date/Time or numeric variable using a 'VB like' format syntax instead of the PHP syntax.
    ...
    if (strpos($FrmStr, '|') !== false) {
        ...
        if (isset($_tbs_FrmMultiLst[$FrmStr])) {
            ...
            if (($FrmNbr <= 2) or ($FrmLst[2] === '')) {
                $FrmLst[2] =& $FrmLst[0];
            } // zero
            if (($FrmNbr <= 3) or ($FrmLst[3] === '')) {
                $FrmLst[3] = '';
            } // null
    ...
    if (!isset($_tbs_FrmSimpleLst[$FrmStr])) {
        tbs_Misc_FormatSave($FrmStr);
    }
...
function tbs_Misc_FormatSave(&$FrmStr)
{
    ...
    if ($nPosEnd !== false) {
        ...
        $_tbs_FrmSimpleLst[$FrmStr] = array(
            'type' => 'num',
            'Pos' => ($nPosCurr + 1),
        ...
        for ($i = 0; $i < $iMax; $i++) {
            ...
            if ($StrIn) {
                ...
                if (($FrmStr[$i] === '"') or ($FrmStr[$i] === '\'')) {
                    ...
                    $Cnt++;
                    if (strcasecmp(substr($FrmStr, $i, 4), 'yyyy') === 0) {
                        $FrmPHP .= 'Y';
                        $FrmLOC .= '%Y';
                        $i += 3;
                        ...
                        $FrmPHP .= 'j';
                        $FrmLOC .= '%d';
                    } elseif (strcasecmp(substr($FrmStr, $i, 2), 'hh') === 0) {
                        $FrmPHP .= 'H';
                        $FrmLOC .= '%H';
                        $i += 1;
...
//Modify $Loc, return false if tags not found, returns the source of the locator if $ReturnSrc=true
    ...
    $RetVal = true;
    $Encaps = 1;
    if ($IsBlock and isset($Loc->PrmLst['encaps'])) {
    ...
    if ($IsBlock) {
        ...
        if ($Loc->PosBeg > $TagO->PosEnd) {
            $RetVal .= substr($Txt, $TagO->PosEnd + 1, min($Loc->PosBeg, $TagC->PosBeg) - $TagO->PosEnd - 1);
        }
        if ($Loc->PosEnd < $TagC->PosBeg) {
            $RetVal .= substr($Txt, max($Loc->PosEnd, $TagO->PosEnd) + 1,
                $TagC->PosBeg - max($Loc->PosEnd, $TagO->PosEnd) - 1);
        }
...
// At this point, the Locator is already merged with $SelValue.
    ...
    if (isset($Loc->PrmLst['selbounds'])) {
        $MainTag = $Loc->PrmLst['selbounds'];
    }
    ...
    if ($TagO !== false) {
        ...
        if ($TagC !== false) {
            ...
            while ($ItemLoc = tbs_Html_FindTag($MainSrc, $ItemTag, true, $Pos, true, 0, true)) {
                ...
                if ($ItemValue !== false) {
                    ...
                    if ($SelArray === false) {
                        ...
                        if (array_search($ItemValue, $SelArray, false) !== false) {
                            $Select = true;
                        }
                    }
                    // Select the item
                    if ($Select) {
                        if (!isset($ItemLoc->PrmLst[$ItemPrm])) {
                            ...
                            if ($IsList and ($ItemLoc->PosBeg < $Item0Beg)) {
                                $Item0Beg = $Item0Beg + strlen($ItemPrmZ);
                            }
                        }
                        $SelNbr++;
                    }
            ...
            if ($IsList) {
                ...
                if (($SelArray === false) and ($SelNbr == 0)) {
                    $MainSrc = $MainSrc . $Item0Src;
                }
                $NewEnd = $TagO->PosEnd;
            } else {
                $NewEnd = $Loc->PosBeg;
    ...
    while ($FileName = $DirObj->read()) {
        ...
        if (strtolower(filetype($FullPath)) === 'file') {
            if (strlen($FileName) >= strlen($Mask)) {
                if ((substr($FileName, 0, $PosL) === substr($Mask, 0, $PosL)) and (substr($FileName,
                            -$PosR) === substr($Mask, -$PosR))
                ) {
                    $FileLst[] = $FullPath;
                }
            }
