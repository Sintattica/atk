...
class FrontController implements \ArrayAccess
{
    ...
    protected function getActionCacheFile()
    {
        ...
        $file = new TmpFile($path . '/' . $filename);
        return $file;
    }
    ...
    protected function handleRequest($request)
    {
        try {
            ...
            $this->m_request = new DataHolder($request);
            ...
            if ($this->m_partial && $this->isRoot()) {
                ...
                if (!isset($this->m_contentType) || $this->m_contentType == 'text/html') {
                    echo Debugger::getInstance()->renderDebugAndErrorMessages();
                }
                die;
            }
        } catch (Exception $ex) {
    ...
    {
        if ($this->isRoot()) {
            Tools::atkerror($exception->__toString());
    ...
    protected function render($template = null, $vars = null, $partial = false)
    {
        ...
        if (!$partial && file_exists(Module::moduleDir($this->m_module) . "scripts/{$this->m_name}.js")) {
            $this->registerScriptFile(Module::moduleDir($this->m_module) . "scripts/{$this->m_name}.js");
        }

        if (!$partial && file_exists(Module::moduleDir($this->m_module) . "styles/{$this->m_name}.js")) {
            $this->registerStyleSheet(Module::moduleDir($this->m_module) . "styles/{$this->m_name}.js");
        }
    ...
    protected function getTemplatePath($template, $directory = 'templates')
    {
        ...
        if (file_exists($root . "$directory/$template")) {
            ...
            if (file_exists(Module::moduleDir($this->m_module) . "$directory/$template")) {
                return Module::moduleDir($this->m_module) . "$directory/$template";
            } else {
                if (file_exists(Module::moduleDir($this->m_module) . "skel/$directory/$template")) {
                    return Module::moduleDir($this->m_module) . "skel/$directory/$template";
                } else {
                    return null;
                }
            }
