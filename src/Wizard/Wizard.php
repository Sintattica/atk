 ...
 *
 * atkWizard's highest baseclass is Controller. When an atkWizard
...
class Wizard extends WizardBase
{
    ...
    /**
     * Value needed by Page to render the content of a page. Default
    ...
    {
        Tools::atkdebug("Wizard::constructor" . Tools::atkGetPostVar("atkcontroller"));
     ...
     * this function. This function overrides Controller::handleRequest but it always
    ...
    public function handleRequest()
    {
        ...
        if (!$this->m_isWizardInitiated && Tools::atkGetPostVar("atkaction") !== "" && Tools::atkGetPostVar("atkwizardaction") === "") {
            ...
            if ((Tools::atkGetPostVar("atkaction") == 'save' || Tools::atkGetPostVar("atkaction") == "update" || Tools::atkGetPostVar("atkaction") == "delete") && $node->filledInForm()) {
                //save form
    ...
    protected function finish()
    {
        ...
        $content = "<br><br>" . Tools::atktext("content_wizard_finished") . "<br><br><br>";

        $page = &$node->getPage();
        $page->addContent($this->genericPage(Tools::atktext("title_wizard_finished"), $content));
    ...
    public function &getCurrentPanel()
    {
        if (!is_object($this->m_panelList[$this->m_currentPanelIndex])) {
            Tools::atkerror("Panel could not be crated. Non existing panel index: " . $this->m_currentPanelIndex);
            return null;
        }
    ...
    /**
     * Set the value of the html meta tag. This is an override of the Controller::setHtmlTitle.
    ...
    public function setHtmlTitle()
    {
        $node = $this->getNode();
        $ui = &$node->getUi();
        return Tools::atktext('app_shorttitle') . " - " . $ui->getWizardTitle($this, $this->getCurrentPanel());
    }

    /**
     * Return the title to be show on top of an Action Page. This is an override of the Controller::actionPageTitle function.
     * We now want to show the wizard name, wizard panel and which step we are at.
     *
