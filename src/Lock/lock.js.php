...
global $ATK_VARS;
$id = (int) $ATK_VARS["id"];
$message = Tools::atktext("lock_expired");
...
function atkLockCheck() {
    var sURI = '<?php echo Tools::session_url('include.php?file=atk/lock/lockimg.php&type=xml&stack=' . $stack); ?>&id=' + atkLock.theIdentifier + '&sequence=' + atkLock.theSequence;
    ...
    if (atkLock.type == 'xml') {
        ...
        image.src = '<?php echo Tools::session_url('include.php?file=atk/lock/lockimg.php&type=image&stack=' . $stack); ?>&id=' + atkLock.theIdentifier + '&sequence=' + atkLock.theSequence;
    }
}
...
function atkLockUnlock() {
    ...
    if (typeof(document.images['_lock_']) != 'undefined')
        document.images['_lock_'].src = '<?php echo Config::getGlobal("atkroot"); ?>atk/images/lock_expired.gif';
