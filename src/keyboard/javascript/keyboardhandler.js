...
atkFEKeyListener.prototype.handleKey = function(key, ctrl, shift)
{
    if (!ctrl)
    {
        if (key == KEY_DOWN && this.onDown)
            kb_focusNext();
        if (key == KEY_UP && this.onUp)
            kb_focusPrevious();
        if (key == KEY_LEFT && this.onLeft)
            kb_focusPrevious();
        if (key == KEY_RIGHT && this.onRight)
            kb_focusNext();
    }
    else
    {
        if ((key == KEY_DOWN || key == KEY_RIGHT) && this.onCtrl)
            kb_focusNext();
        if ((key == KEY_UP || key == KEY_LEFT) && this.onCtrl)
            kb_focusPrevious();
    }
}
...
function kb_focusLast()
{
    kb_defocus();
    if (keyListeners.length > 0)
        focussedListener = keyListeners.length - 1;
