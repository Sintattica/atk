 ...
 *
 */
class SecureRelation extends OneToOneRelation
{
    var $m_crypt = null;
    var $m_linktable;
    var $m_linkfield;
    ...
    function getLinkPassword($linktable, $linkfield, $encryption = "")
    {
        ...
        if ($encryption) {
            $crypt = Encryption::getEncryption($encryption);
        } else {
            $crypt = $this->m_crypt;
        }
        ...
        if ($username == "administrator") {
            ...
            $query = "SELECT " . $linkfield . " as pass FROM " . $linktable . " WHERE " . Config::getGlobal("auth_userfield") . " = '" . $username . "'";
    ...
    function load(&$db, $record, $mode)
    {
        if ($this->m_searching) {
                ...
                if ($this->createDestination()) {
                    ...
                    for ($i = 0, $_i = count($records); $i < $_i; $i++) {
                        ...
                        if (!$decryptedlink && $errorconfig) {
                            Tools::atkdebug("Unable to decrypt link: " . $link . "for record: " . var_export($records[$i],
                    ...
                    if ($decrypterror) {
                        if ($errorconfig == 2) {
                            Tools::atkerror("There were errors decrypting the secured links, see debuginfo");
                        } else {
                            if ($errorconfig == 1) {
                                Tools::mailreport();
                            }
            ...
            if ($cryptedlink) {
                ...
                Tools::atkdebug("Could not decrypt the link: $link for " . $this->m_ownerInstance->primaryKeyField() . "='" . $record[$this->m_ownerInstance->primaryKeyField()]);
            }
        }
    }
            ...
            if ($this->m_ownerInstance) {
                if ($this->m_cachefield && is_numeric($cryptedlink) && $cryptedlink && $record[$this->m_ownerInstance->primaryKeyField()] && $this->createDestination()) {
                    ...
                    if (!$cryptedlink || !is_numeric($cryptedlink)) {
                        Tools::atkdebug("decrypt($record, $field) failed! and yielded: $cryptedlink");
                        return null;
                    }
                }
            } else {
                Tools::atkhalt("no ownerinstance found for the secure relation");
            }
        } else {
