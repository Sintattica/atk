...
class ManyToOneRelation extends Relation
{
    ...
    {
        if (!$this->createDestination()) {
            return Tools::atkerror("Could not create destination for destination: $this -> m_destination!");
        }
    ...
    function _getSelectLink($selname, $filter)
    {
        ...
        $atktarget = Tools::atkurlencode(Tools::getDispatchFile() . "?atklevel=" . SessionManager::atkLevel() . "&" . $selname . "=[atkprimkey]");
                ...
                $autolink['add'] = "&nbsp;" . Tools::href(Tools::dispatch_url($this->getAutoLinkDestination(),
                        "add", array(
                            ...
                            "atkfilter" => ($this->m_useFilterForAddLink && $filter != ""
                                ? $filter : "")
                        )), Tools::atktext("new"), SESSION_NESTED, true, 'class="atkmanytoonerelation"');
            }
        }
        return $autolink;
    }
