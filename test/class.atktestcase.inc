...
abstract class atkTestCase extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        if ($this->useTestDatabase) {
            ...
            $mapping = atkConfig::getGlobal("test_db_mapping");
            foreach ($mapping as $test) {
                $testDb = atkTools::atkGetDb($test);
    ...
    protected function _validateTestDb()
    {
        ...
        foreach (array_keys($db) as $normal) {
            ...
            $conn = atkTools::atkGetDb($mapping[$normal]);
    ...
    {
        atkTools::atkdebug("Load fixtures into test database...");

        $db = $this->_getTestDb($database);
        $db->toggleForeignKeys(false);

        atkTools::atkimport('atk.fixture.atkfixturemanager');
    ...
    function __call($method, $args)
    {
        foreach (array_keys($this->m_fixtureData) as $database) {
            if (isset($this->m_fixtureData[$database][$method]))
                return $this->fixture($method, isset($args[0]) ? $args[0] : NULL, isset($args[1])
                                ? $args[1] : NULL);
        }
    ...
    {
        return atkTools::atkGetDb($database);
    }
    ...
    {
        $this->m_restoreNode[$nodename] = &atkModule::atkSetNode($nodename, $mocknode);
    }

    function restoreNode($nodename)
    {
        atkModule::atkSetNode($nodename, $this->m_restoreNode[$nodename]);
    }
    ...
    function &setMockSecurityManager(&$mockmanager)
    {
        ...
        $g_securityManager = $mockmanager;
        $this->m_restoreSecMgr = $obj;
    }

    function restoreSecurityManager()
    {
        global $g_securityManager;
        $g_securityManager = $this->m_restoreSecMgr;
    }
