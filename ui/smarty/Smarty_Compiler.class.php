...
class Smarty_Compiler extends Smarty
{
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        for ($curr_tb = 0, $for_max = count($text_blocks); $curr_tb < $for_max; $curr_tb++) {
            ...
            if (preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $text_blocks[$curr_tb],
                $sp_match)) {
    ...
    function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        if ($start_tag) {
            ...
            if ($this->_pop_tag($tag_command) === false) {
                return true;
            }
        }
                ...
                default:
                    if (preg_match('~^' . $this->_func_regexp . '$~', $token)) {
                        ...
                        if ($this->security &&
                            ...
                            $token) && isset($tokens[$i + 1]) && $tokens[$i + 1] == '('
                    ) {
        ...
        foreach ($tokens as $key => $token) {
            switch ($state) {
                case 0:
                    ...
                    if (preg_match('!^((?>\w|#|/|\$)(\w|@|-|_|:|#|/|\.)*|[\'"](?>\w|#|/|\$)(\w|@|-|_|\?|&|=|:|#|/|\.|\s)*[\'"])$!',
                        $token)) {
                        $attr_name = $token; // Smarty will autostrip the quotes when needed
                        $state = 1;
                    } else {
                        $this->_syntax_error("invalid attribute name: '$token' for tokens: " . var_export($tokens,
                                1) . " with key: $key", E_USER_ERROR, __FILE__, __LINE__);
                    }
    ...
    function _expand_quoted_text($var_expr)
    {
        ...
        if (preg_match_all('~(?:\`(?<!\\\\)\$' . $this->_dvar_guts_regexp . '(?:' . $this->_obj_ext_regexp . ')*\`)|(?:(?<!\\\\)\$\w+(\[[a-zA-Z0-9]+\])*)~',
            ...
            foreach ($_match as $_var) {
                $var_expr = str_replace($_var, '".(' . $this->_parse_var(str_replace('`', '', $_var)) . ')."',
                    $var_expr);
            }
