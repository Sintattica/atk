<table id="editform" border="0">
  ...
  {foreach from=$fields item=field}
    <tr{if $field.rowid != ""} id="{$field.rowid}"{/if}{if $field.initial_on_tab!='yes'} style="display: none"{/if} class="{$field.tab}">
      ...
      {else}
      {if $field.label!=="AF_NO_LABEL"}<td valign="top" class="{if $field.error!=""}errorlabel{else}fieldlabel{/if}">{if $field.label!=""}<b>{$field.label}</b>:  {if $field.obligatory!=""}{$field.obligatory}{/if}{/if}</td>{/if}
<!--    </tr>
    <tr{if $field.rowid != ""} id="{$field.rowid}"{/if}{if $field.initial_on_tab!='yes'} style="display: none"{/if} class="{$field.tab}"> -->
